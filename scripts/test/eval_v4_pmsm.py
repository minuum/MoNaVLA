...
def eval_v4():
    ...
    # NavTrainer is NavTrainer (which inherits from MobileVLATrainer)
    trainer = NavTrainer.load_from_checkpoint(
        checkpoint_path,
        configs=config,
        map_location=device,
        strict=False # LoRA checkpoints often have missing keys for the frozen part
    )
    ...
    for i in tqdm(range(total_samples)):
        ...
        # In NavH5DatasetImpl.__getitem__, rgb is (window+chunk, C, H, W) = (5, 3, 224, 224)
        # We need the first 4 frames for the window
        window_size = config["train_dataset"]["window_size"]
        rgb = data['rgb'][:window_size].unsqueeze(0).to(device) # (1, 4, 3, 224, 224)
        lang = data['lang']
        ...
        # output is (B, L, chunk, num_classes) = (1, 1, 1, 9)
        ...
        # Manual loss calculation for debugging
        target = torch.tensor([true_class], device=device)
        sample_loss = torch.nn.functional.cross_entropy(pred_logits.unsqueeze(0), target).item()
        ...
        # Store some details for analysis
        if i < 3: # Detail for first 3
            print(f"Sample {i} | Loss: {sample_loss:.4f} | Logits: {pred_logits.detach().cpu().numpy()}")
    ...
    print("\n" + "="*70)
    print("📈 Evaluation Results (V4-EXP01)")
    print("="*70)
    ...
    # In NavH5DatasetImpl, there is a mapping in self.classes or similar.
    # Let's just print the counts.
    for c in sorted(true_counts.keys()):
        print(f"  Class {c}: {true_counts[c]} samples")
    print("-" * 70)
    print(f"{'ID':<3} | {'Pred Class':<10} | {'True Class':<10} | {'Match':<5}")
    for r in results:
        print(f"{r['id']:<3} | {r['pred']:<10} | {r['true']:<10} | {r['match']:<5}")
    print("="*70)
